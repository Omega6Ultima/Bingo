...
public:
	...
	static inline float ms_to_sec(uint milliseconds){
		return milliseconds / 1000.0f;
	}

	static inline uint sec_to_ms(float seconds){
		return static_cast<uint>(seconds * 1000.0f);
	}

	static inline float sec_to_min(uint sec){
		return sec / 60.0f;
	}

	static inline uint min_to_sec(float min){
		return static_cast<uint>(min * 60.0f);
	}

	static inline float min_to_hour(uint min){
		return min / 60.0f;
	}

	static inline uint hour_to_min(float min){
		return static_cast<uint>(min * 60.0f);
	}
private:
