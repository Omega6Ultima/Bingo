...
void ThreadManager::startThread(string threadName) {
	if (threads.find(threadName) != threads.end()) {
		sdl_threads[threadName] = SDL_CreateThread(InternalUse::anotherLayerOfAbstraction, threadName.c_str(), &threads[threadName]);
	}
}
...
static int Bingo::InternalUse::anotherLayerOfAbstraction(void* data) {
